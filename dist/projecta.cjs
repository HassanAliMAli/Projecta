...
var ProjectDetector = class {
  ...
  static detectProject() {
    ...
    try {
      const gitDir = (0, import_child_process.execSync)("git rev-parse --show-toplevel", {
        cwd,
        encoding: "utf-8",
        stdio: "pipe"
        // Suppress command output on stderr
      }).trim();
      const projectName = import_path2.default.basename(gitDir);
      return {
        name: projectName,
        path: gitDir,
        type: "git"
      ...
      return {
        name: import_path2.default.basename(cwd),
        path: cwd,
        type: "folder"
      };
    }
...
var storeMemoryTool = {
  ...
  async handler(args, { sqliteStorage: sqliteStorage2, samplingHelper: samplingHelper2 }) {
    const { content, importance } = args;
    const projectInfo = ProjectDetector.detectProject();
    const project = projectInfo.name;
    const tags = await samplingHelper2.generateTags(content);
    const semanticKeywords = await samplingHelper2.extractKeywords(content);
    const id = sqliteStorage2.insertMemory({
      content,
      tags,
      semanticKeywords,
      importance,
...
var searchMemoryTool = {
  ...
  async handler(args, { sqliteStorage: sqliteStorage2, samplingHelper: samplingHelper2 }) {
    ...
    const expandedQuery = await samplingHelper2.expandQuery(query);
    const results = sqliteStorage2.searchMemories({
      query: expandedQuery,
      project,
      limit: 5
      // Limit to 5 results as per MVP spec
    });
    if (results.length === 0) {
      return {
...
var listProjectsTool = {
  ...
  async handler(args, { sqliteStorage: sqliteStorage2 }) {
    const projects = sqliteStorage2.listProjects();
...
// src/resources/project-context.js
async function registerProjectContextResource(server2, storage) {
  server2.setRequestHandler("resources/list", async () => {
    return {
      resources: [
        {
  ...
  server2.setRequestHandler("resources/read", async (request) => {
